...
- Name: 07-videos.spec.ts >> 07 — Vídeos >> channel meta shows CREMOSA + @cremos4 + video count
- Location: tests/e2e/07-videos.spec.ts:45:7
...
Locator: locator('.win95-title-text').filter({ hasText: 'CREMOSA' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.win95-title-text').filter({ hasText: 'CREMOSA' }).first()
