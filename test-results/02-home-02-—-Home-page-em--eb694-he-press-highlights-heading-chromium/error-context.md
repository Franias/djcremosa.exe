...
- Name: 02-home.spec.ts >> 02 — Home page >> em destaque section shows the press highlights heading
- Location: tests/e2e/02-home.spec.ts:149:7
...
Call log:
  - waiting for getByRole('heading', { name: /Dez anos na pista/i, level: 2 })
...
> 156 |       .scrollIntoViewIfNeeded();
  ...
  223 |     context,
  224 |   }) => {
  225 |     await context.grantPermissions(["clipboard-read", "clipboard-write"]);
  226 |     await page
  227 |       .locator("text=cremosa.txt — readme")
  228 |       .first()
  229 |       .scrollIntoViewIfNeeded();
  230 |     await page.getByRole("button", { name: /copiar/i }).first().click();
  231 |     await expect(
  232 |       page.getByText(/✓ copiado|copiar ✓/i).first(),
  233 |     ).toBeVisible({ timeout: 3_000 });
  234 |     const clipboardText = await page.evaluate(() =>
  235 |       navigator.clipboard.readText(),
  236 |     );
  237 |     expect(clipboardText).toContain("DJ Cremosa");
  238 |     expect(clipboardText).toContain("música preta global");
  239 |   });
  240 | 
  241 | });
  242 | 
